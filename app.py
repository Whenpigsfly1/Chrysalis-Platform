...
app.config['MYSQL_PASSWORD'] = '666witches'
app.config['MYSQL_DB'] = 'Chrystest'
...
def t_create():
    if request.method == "POST":
        ...
        cur.execute("INSERT INTO schools(school_name, t_username, t_password) VALUES (%s, %s, %s)",
...
def t_login():
    ...
    if request.method == "POST":
        ...
        school = session["teacher_school"]
        cur = mysql.connection.cursor()
        cur.execute("SELECT * FROM schools WHERE school_name = %s AND t_username = %s AND t_password = %s",
                    (school, username, password))
        User = cur.fetchone()
        cur.close()
        print(User)
...
def t_home():
    ...
    videos = cur.fetchall()
    cur.execute("SELECT Student_username FROM teachers2 WHERE School_name =%s AND Teacher_username =%s",
                (school, teacher_username))
    Students = cur.fetchall()
    mysql.connection.commit()
    cur.close()
...
def add_student():
    if request.method == "POST":
        ...
        cur.execute("INSERT INTO teachers2(School_name, Teacher_username, Student_username) VALUES (%s, %s, %s)",
                    (school, teacher_username, added_student_username))
        mysql.connection.commit()
        cur.close()
...
def assign_to_all():
    ...
    if request.method == "POST":
        ...
        for student in students:
            cur.execute(
                "INSERT INTO videos2 (Student_school, Teacher_name, Student_username, Video_Name, Completed) VALUES (%s,%s,%s,%s,%s)",
                (school, teacher, student, assigned, 0))
            mysql.connection.commit()
        cur.close()
        return redirect('/t_home')
...
def t_student_layout():
    ...
    cur = mysql.connection.cursor()
    cur.execute(
        "SELECT Video_Name,Q1_answer,Q2_Answer,Q3_Answer,Notes FROM videos2 WHERE Student_school = %s AND Teacher_Name = %s AND Student_username = %s AND completed = 0",
        (school, teacher, student))
    unfinished_videos = cur.fetchall()
    print(unfinished_videos)
    cur.execute(
        "SELECT Video_Name,Q1_answer,Q2_Answer,Q3_Answer,Notes,Timestamp FROM attempts WHERE Student_school = %s AND Teacher_Name = %s AND Student_username = %s",
        (school, teacher, student))
    finished_videos = cur.fetchall()
    print(finished_videos)
    return render_template('t_student_layout.html', student=student, unfinished_videos=unfinished_videos,
...
def assign_video():
    ...
    if request.method == "POST":
        ...
        cur.execute(
            "INSERT INTO videos2 (Student_school, Teacher_name, Student_username, Video_Name, Completed) VALUES (%s,%s,%s,%s,%s)",
            (school, teacher, student_username, assigned_video, 0))
        mysql.connection.commit()
        cur.close()
        return redirect("/t_student_layout")
    #cur = mysql.connection.cursor()
...
def s_create():
    ...
    if request.method == "POST":
        ...
        password = details['password']
        cur = mysql.connection.cursor()
        cur.execute("SELECT student_username FROM newstest WHERE school_name= %s AND teacher_name = %s",
                    (school, teacher_name))
        ...
        for student in students:
            if student[0] == username:
                cur.execute(
                    "INSERT INTO newstest(school_name,teacher_name,student_username,password) VALUES (%s,%s,%s,%s)",
                    (school, teacher_name, username, password))
                mysql.connection.commit()
                cur.close()
...
def s_login():
    ...
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "SELECT * FROM newstest WHERE school_name = %s AND teacher_name=%s AND student_username = %s AND password = %s",
            (school, teacher, username, password))
        User = cur.fetchone()
        cur.close()
        print(User)
        if User:
...
def s_home():
    ...
    print(student)
    cur.execute(
        "SELECT Video_Name FROM videos2 WHERE Student_school =%s AND Teacher_Name = %s AND Student_username = %s AND completed = 0",
        (school, teacher, student))
    unfinished_videos = cur.fetchall()
    print('unfinished_videos')
    print(unfinished_videos)
    ...
    cur.execute(
        "SELECT v.Student_username, v.Video_Name, vf.Video_filename FROM videos2 v JOIN video_filenames_test8 vf on v.Video_Name = vf.Video_Number WHERE Student_school =%s AND Teacher_Name = %s AND Student_username = %s AND completed = 0",
        (school, teacher, student))
    ...
    #finished_videos =cur.fetchall()
    return render_template('s_home.html', student=student, unfinished_videos=unfinished_videos,
                           unfinished_video_filenames=unfinished_video_filenames)
...
def show_video():
    ...
    if request.method == "POST":
        ...
        cur.execute(
            "UPDATE videos2 SET Completed = 1 WHERE Student_school =%s AND Teacher_Name = %s AND Student_Username = %s AND Video_name = %s",
            (school, teacher, student, video))
        grade = video_scoring(school, teacher, video_number, q1, q2, q3)
        cur.execute(
            "INSERT INTO attempts (Student_school, Teacher_name, Student_username, Video_Name,Q1_Answer,Q2_Answer,Q3_Answer,Notes,Grades) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            (school, teacher, student, video, q1, q2, q3, notes, grade))
    ...
    cur = mysql.connection.cursor()
    cur.execute(
        "SELECT q1,o1a,o1b,o1c,o1d,q2,o2a,o2b,o2c,o2d,q3,o3a,o3b,o3c,o3d FROM video_filenames_test8 WHERE Video_Number= %s",
        (video_number,))
...
def video1():
    ...
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "UPDATE videos2 SET Completed = 1, Q1_Answer = %s, Q2_Answer = %s, Q3_Answer = %s, Notes = %s WHERE Student_school =%s AND Teacher_Name = %s AND Student_Username = %s AND Video_name = %s",
            (q1, q2, q3, notes, school, teacher, student, video))
        mysql.connection.commit()
        cur.close()
        return redirect("/s_home")
...
def s_videos():
    ...
    #print(unfinished_video_urls)
    cur.execute(
        "SELECT * FROM videos2 WHERE Student_school =%s AND Teacher_Name = %s AND Student_username = %s AND completed = 1",
        (school, teacher, student))
    finished_videos = cur.fetchall()
    return render_template('s_videos.html', student=student, finished_videos=finished_videos,
                           unfinished_video_filenames=unfinished_video_filenames)
...
def s_all_videos():
    ...
    math = cur.fetchall()
    return render_template('s_all_videos.html', biology=biology, technology=technology, earth_science=earth_science,
                           math=math, videos=videos)
...
def t_all_videos():
    ...
    return render_template('t_all_videos.html', biology=biology, technology=technology, earth_science=earth_science,
                           math=math, videos=videos)
...
def insert_video():
    if request.method == "POST":
        ...
        tag = details['tag']
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO video_filenames_test8(Video_Number, Video_filename,Video_tag) VALUES (%s,%s,%s)",
                    (name, url, tag))
        mysql.connection.commit()
        cur.close()
...
def insert_video_questions():
    ...
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "UPDATE video_filenames_test8 SET q1=%s,o1a=%s,o1b=%s,o1c=%s,o1d=%s,q2=%s,o2a=%s,o2b=%s,o2c=%s,o2d=%s,q3=%s,o3a=%s,o3b=%s,o3c=%s,o3d=%s,worksheet_url=%s WHERE Video_Number =%s",
            (q1, o1a, o1b, o1c, o1d, q2, o2a, o2b, o2c, o2d, q3, o3a, o3b, o3c, o3d, worksheet, video_number))
        mysql.connection.commit()
        cur.close()
        return redirect('/')
...
def cells_wkst_1():
    if request.method == "POST":
        ...
        print("open")
        cur.execute(
            "INSERT INTO cells_wkst_1(School_name,Teacher_name,Student_name,answer1,answer2,answer3,answer4,answer5,answer6) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
            (school, teacher, student, answer1, answer2, answer3, answer4, answer5, answer6))
        mysql.connection.commit()
        cur.close()
        print("cured")
...
def video_scoring(Student_school, Teacher_name, Video_Name, Q1_Answer, Q2_Answer, Q3_Answer):
    cur = mysql.connection.cursor()
    cur.execute(
        "SELECT Q1_Answer, Q2_Answer, Q3_Answer FROM video_answers WHERE School_name =%s AND Teacher_username =%s AND Video_Name = %s",
        (Student_school, Teacher_name, Video_Name))
    answers = cur.fetchall()
    video_answers = []
    for answer in answers:
        video_answer = answer[0]
        video_answers.append(video_answer)
    grade = 0
    if video_answers[0] == Q1_Answer:
        grade += 1
    if video_answers[1] == Q2_Answer:
        grade += 1
    if video_answers[2] == Q3_Answer:
        grade += 1
    return grade
